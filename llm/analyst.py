...
from llm.prompts import build_analyst_prompt, build_arbiter_prompt
...
class LLMAnalyst:
    ...
    def analyze_with_intelligence(
        ...
        """
        Analyze using the full intelligence report (all AI model predictions).
        The LLM acts as the final arbiter with all signals visible.
        """
        log.info(f"Analyzing {bundle.symbol} with intelligence report (composite: {bundle.composite_score:.0f})")

        system, user = build_arbiter_prompt(
            bundle=bundle,
